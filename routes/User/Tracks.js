...
const getSpotifyPlaylistTracks = (playlist, limit = 20, offset) => {
  return accessToken().then(spotifyAccess => {
      return axios.get(
...
const spotifyTracksProcessor = async (
  ...
  console.log('stattsssss', {userSideOffset}, {limit}, {finalTracks: finalTracks.length})
  let allTrackProcessed = [];
  let availableTracks = [];
  let processedCount = 0;
  let terminatedAt;
  let spotifyTracksResponse = await getSpotifyPlaylistTracks(playlist, limit, userSideOffset)
    .then(res => res.data)
    .catch(err => {
  ...
  for (let i = 0; i < spotifyTracks.length; i++) {
    ...
    //   break
    // }
    if (spotifyTracks[i].track && query.length && !spotifyTracks[i].track.name.toLowerCase().includes(query.toLowerCase())) {
      console.log('ignored')
    }
    else {
      if (spotifyTracks[i].track) {
        ...
        if (type === 'noMatch') {
          ...
          if (trackMatched) {
            ...
            if (spotifyTracks[i].track.matchWithLocalTracks === false) {
              console.log('Checking the status here', spotifyTracks[i].track.matchWithLocalTracks)
              unAvailableTracks.push(spotifyTracks[i]);
            }
          }
          else {
        ...
        if (type == 'available' && (finalTracks.length + availableTracks.length) == 10) {
          break
        }
        if ((type == 'noMatch' || type == 'unavailable') && (finalTracks.length + unAvailableTracks.length) == 10) {
          break
        }
        if (type == 'all' && (finalTracks.length + allTrackProcessed.length) == 10) {
          break
        }
        // }
        // else {
  ...
  await userAuthVerification(authorization)
    .then(async () => {
      ...
      let getPlaylistDetails =
        await accessToken().then(spotifyAccess => {
          return axios
      ...
      console.log(getUserGroupDetails.searchLimit , finalTracks.finalTracks.length, "-->>>")
      // if (!addToSpotify && (getUserGroupDetails.searchLimit < finalTracks.finalTracks.length)) {
      //   return res
      //     .status(400)
      //     .json(
      //       response(
      ...
      console.log(maintainUnavailable, 'unaval')
      ...
      //         }
      //         if (remaining) {
      //           if (allTracks[i].track && (allTracks[i].track.matchWithLocalTracks != null ||
      // allTracks[i].track.matchAtall  != null)) {  // { //   filterByLicencedPublishers: false, //
      // filterByLicencedLabels: false, //   filterByLicencedPROs: false // } let filteredTrack = await
      // generateLogsForTrack( allTracks[i].track, userPreferences, type, getUserGroupDetails, availableTracks); //
      // if
      // (filteredTrack.availableTracks) { allTracks[i].track = filteredTrack.track; if
      // (filteredTrack.track.matchWithLocalTracks === true) { availableTracks = filteredTrack.availableTracks } if
      // (allTracks[i].track.matchWithLocalTracks === false) { unAvailableTracks.push(allTracks[i]); } } else { if
      // (allTracks[i].track) { allTracks[i].track.newFormatLogReason.noMatch = 'No Match'
      // unAvailableTracks.push(allTracks[i]); } console.log('straight away not available', i) } if (type == 'all') {
      // console.log('In all ', i) // remaining -= 1 allTrackProcessed.push(allTracks[i]) } } --remaining; if
      // (!remaining) { terminatedOnIndex = i + 1; break; } // console.log('=====> newFormatLogReason >>',
      // allTracks[i].track.newFormatLogReason) } // return res.json("Killed")
      // console.log('simpleUnMatchWithSpotifyTracks', unAvailableTracks.length); // save all not matched track in
      // logs collection if (unAvailableTracks.length && type != 'available') {//unAvailableTracks.length is postive
      // let destructUnavailableTracks = []; unAvailableTracks.forEach(item =>
      // destructUnavailableTracks.push(item.track)) let historyObject = new History({ email: userByAuth.email,
      // _group: getUserGroupDetails._id, query: { playlist_name: getPlaylistDetails.name }, _track:
      // destructUnavailableTracks, success: false, type: 'SpotifyTrackNotAvailable' }); await historyObject.save();
      // }  // save all not matched track in logs collection // if (allTrackNotPassedWithPercentage.length) { //
      // let historyObject = new History({ //     email: userByAuth.email, //     _group: getUserGroupDetails._id, //
      //     query: { //       playlist_name: getPlaylistDetails.name //     }, //     _track:
      // allTrackNotPassedWithPercentage, //     success: true, // type: 'SpotifyTrackNotAvailable' //   }); //
      // await historyObject.save(); // }  // decrease query limit counter let updateUserQueryCount = await
      // User.findOneAndUpdate( {_id: _user}, { $inc: { query_count: -allTrackProcessed.length } }, {new: true}
      // ).catch((err) => { return response('SWR', 'Search count not updated', null, err); });  // update user last
      // search time to prevent attacks let updateUserLastSearchTime = await User.findOneAndUpdate( {_id: _user}, {
      // $set: { lastSearchAt: Date.now() } } ).catch((err) => { return res .status(400) .json(response('SWR', 'Try
      // again.', null, null)); }); console.log(allTracks.length); let finalTracks = []; if (type == 'unavailable') {
      // finalTracks = unAvailableTracks } else if (type == 'available') { finalTracks = availableTracks
      // console.log(finalTracks.length, "=>>>>>>") } else { console.log('all ') finalTracks = allTrackProcessed }
      // console.log({finalTracks: finalTracks.length})  if (!terminatedOnIndex) terminatedOnIndex = allTracks.length
      // return res .status(200) .json( response( 'S', 'successful', {totalTrackCount, totalTracks: officialLength,
      // tracks: finalTracks, terminatedOnIndex}, null ) ); } else { return res .status(400) .json(response('SWR',
      // 'Group details invalid.', null, null)); } } else { return res.status(400).json(response('SWR', 'Invalid
      // auth.')); }  }) .catch((err) => { console.log('err', err.message) return res.status(400).json(response('PD',
      // null, null, err)); });
    })
    .catch(err => {
      return res
...
router.post('/filterNewOld', async (req, res) => {
  let {authorization} = req.headers;
  let {playlist, _user, type, allTracks, remaining, userPreferences, addToSpotify} = req.body;
      ...
      if (userByAuth) {
        ...
        if (getUserGroupDetails) {
          for (let i = 0; i < allTracks.length; i++) {
            ...
            if (remaining) {
              if (allTracks[i].track && (allTracks[i].track.matchWithLocalTracks != null || allTracks[i].track.matchAtall != null)) {
          ...
          if (unAvailableTracks.length && type != 'available') {//unAvailableTracks.length is postive
            let destructUnavailableTracks = [];
            unAvailableTracks.forEach(item => destructUnavailableTracks.push(item.track))
            let historyObject = new History({
              email: userByAuth.email,
              _group: getUserGroupDetails._id,
              query: {
          ...
          return res
            ...
            .json(
              response(
                'S',
                'successful',
                {totalTrackCount, totalTracks: officialLength, tracks: finalTracks, terminatedOnIndex},
                null
              )
            );
        }
...
router.post('/filter', async (req, res) => {
  ...
  await userAuthVerification(authorization)
    .then(async () => {
      ...
      await axios
        .get(`${spotify_url}/playlists/${playlist}/tracks?limit=${trackLimit}&offset=${(page - 1) * 10}`, {
        ...
        .then(async function (tracks) {
          ...
          if (userByAuth) {
            ...
            if (getUserGroupDetails) {
              ...
              for (let i = 0; i < allTracks.length; i++) {
                if (allTracks[i].track && allTracks[i].track.matchWithLocalTracks === true) {
              ...
              console.log('simpleUnMatchWithSpotifyTracks', simpleUnMatchWithSpotifyTracks.length);
              // save all not matched track in logs collection
              ...
              return res
                ...
                .json(
                  response(
                    'S',
                    'successful',
                    {totalTrackCount, totalTracks: officialLength, tracks: allTracks},
                    null
                  )
                );
            }
    ...
    .then(async () => {
      ...
      await axios
        ...
        .then(async function (tracks) {
          ...
          // const allMatchingTracks = await AvailableTracks.aggregate([
          //   {
          //     $match: {
          //       $or: [
          //         {
          ...
          // const nonMatchingTracks = await AvailableTracks.aggregate([
          //   {
          //     $match: {
          //       $and: [
          //         { isrc: { $nin: allTrackISRCs } },
          //         {
          //           $or: [
          //             {
          //               $and: [
          //                 { artist: { $nin: allTrackISRCs } },
          //                 { title: { $nin: allTitles } },
          //               ],
          //             },
          //             {
          ...
          //   {
          //     $addFields: {
          //       matchWithLocalTracks: false,
          //     },
          //   },
          // ]);

          // console.log(nonMatchingTracks);

          let filteredTracks = [];
          // console.log(allTracks[0].track.external_ids.isrc);
          // matchedAndUnMatched.forEach((mau) => {
          //   allTracks.forEach((item) => {
          //     if (item.track.external_ids.isrc == mau.isrc) {
          ...
          // console.log(allMatchingTracks);
          console.log(allMatchingTracks);
          console.log(allTrackISRCs);
          ...
          // console.log(matchedAndUnMatched);
          // console.log(allTracks);

          // const nonMatchingTracks = await AvailableTracks.find({
          //   $and: [
          //     { isrc: { $nin: allTrackISRCs } },
          //     {
          //       $or: [
          ...
          // console.log(nonMatchingTracks, "none matching");

          // console.log(allArtists);

          // traverse all tracks under admin spotify playlist
          // for (let i = 0; i < allTracks.length; i++) {
          //   let titleAndArtisMatch = [];
          //   allTracks[i].track.matchWithLocalTracks = false;
...
//         allTracks[i].track.matchWithLocalTracks = true;
//         if (type == 'available') {
//           availableTracks.push(allTracks[i])
//         }
//         isUnavailable = false
//         break;
//       }
