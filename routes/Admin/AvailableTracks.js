...
router.post("/", uploadCSV.single("availableTracks"), async (req, res) => {
  ...
  try {
    const PATH = `./dataSet/${req.file.filename}`;

    io.emit("trigger", {
      message: "parsing file",
    });

    const docs = await readfile(PATH);
...
router.post(
  ...
  async (req, res) => {
    const { io } = require("../../server");
    // console.log('hadiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii');
    io.emit("trigger", {
      message: "file uploaded",
    });
    ...
    try {
      ...
      console.log(rows.length, "--0", user_group.searchLimit);
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        console.log(rowData.length, "----------->>> legth");
      ...
      console.log(logItems.length, "logItems <<<---------------");
